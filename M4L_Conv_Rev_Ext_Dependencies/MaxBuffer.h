...
public:
    ...
    void setSampleRate(double sampleRate)
    {
        ...
        t_atom temp_atom[2];
        atom_setfloat(temp_atom, sampleRate);
        object_method_typed(getBuffer(), gensym("sr"), 1L, temp_atom, temp_atom + 1);
    }
    
    void resize(t_atom_long length)
    {
        release();
        
        t_atom temp_atom[2];
        atom_setlong(temp_atom, length);
        object_method_typed (getBuffer(), gensym("sizeinsamps"), 1L, temp_atom, temp_atom + 1);
        
        acquireInternal();
    }
    
    operator bool()                 { return getBuffer(); }
    ...
    template <typename T>
    void write(const T *input, t_ptr_uint length, t_ptr_uint offset, t_ptr_uint chan, double mul)
    {
        ...
        for (size_t i = 0; i < length; i++, samples += mChans)
            *samples = static_cast<float>(input[i] * mul);
        
        setDirty();
    }
    
    void writeZeros(t_ptr_uint length, t_ptr_uint offset, t_ptr_uint chan)
    {
        length = constrainLength(length, chan, offset);
