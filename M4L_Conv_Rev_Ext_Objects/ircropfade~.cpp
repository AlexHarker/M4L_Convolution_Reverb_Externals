...
// - crop 2 (samps)
// - fade in start (samps)
// - fade in length (samps)
// - fade out end (samps)
// - fade out length (samps)
...
void ircropfade_process_internal(t_ircropfade *x, t_symbol *sym, short argc, t_atom *argv)
{
    ...
    t_ptr_int full_length = buffer_length(source);
    ...
    t_ptr_int final_length = crop2 - crop1;

    // Allocate Memory

    temp_ptr<float> temp1(full_length);
    temp_ptr<double> temp2(final_length);
    ...
    buffer_read(source, read_chan, temp1.get(), full_length);

    // Copy with crops / fades to double precision version

    for (t_ptr_int i = 0; i < final_length; i++)
    {
